# OSMP Protocol Specification v1.0.2
...
2. Each opcode encodes to a 2-byte value: the namespace byte followed by a dictionary index byte derived from the base ASD. Opcode index assignment is deterministic across all conforming nodes that share a base ASD fingerprint and is independent of the broader basis composition.
...
The development workflow: compose and debug in SAL (human-readable), deploy in SAIL (binary, maximum compression) for any peer pair with a matching basis, decompile the wire payload back to readable SAL for inspection at any point. Peer pairs that do not share a basis fall through to SAL on the wire — bijective and inspectable, with the marginal compression difference between SAL and SAIL traded for unconditional cross-basis interop.
...
SEC envelope structure (87 bytes fixed overhead for 2-byte node_id, 89 bytes for 4-byte node_id):

| Offset | Size | Field | Description |
|---|---|---|---|
| 0 | 1B | mode_byte | Bits 0-1: wire mode (00=MNEMONIC, 01=SAIL, 10=SEC, 11=SAIL_SEC). Bit 2: node_id length (0=2B, 1=4B). Bits 5-7: SEC version (000=v1) |
| 1 | 2B or 4B | node_id | Sender node identifier (length determined by mode_byte bit 2) |
| 1+N | 4B | sequence | Monotonic sequence counter (big-endian u32, never reused per node) |
| 5+N | variable | sealed_payload | ChaCha20-Poly1305 ciphertext of the SAL or SAIL encoded instruction |
| end-80 | 16B | auth_tag | ChaCha20-Poly1305 authentication tag |
| end-64 | 64B | signature | Ed25519 signature over (header || sealed_payload || auth_tag) |

Where N = 2 or 4 depending on the node_id length flag.

**Nonce derivation (not on the wire)**: the 12-byte AEAD nonce is derived deterministically from the envelope header (mode_byte || node_id || sequence) padded with the canonical 12-byte OSMP nonce salt `OSMP-SEC-v1\x00`. The nonce is NOT transmitted: both sides reconstruct it from the header bytes they already share. This saves 12 bytes per envelope and tightens the binding between the header, the AEAD, and the sequence counter.

**Replay protection**: the sequence counter provides replay protection. Each node maintains a monotonic counter that increments per transmitted message and never resets. The receiving node tracks the highest sequence number seen from each peer and rejects any message with a sequence number equal to or lower than the recorded maximum. The 32-bit counter accommodates 4.29 billion messages per node before requiring rekey.

**Authentication**: the Ed25519 signature authenticates the sender without a certificate authority. Nodes exchange public keys during FNP handshake or via out-of-band provisioning. The signature covers the full envelope header, the sealed payload, and the auth tag, preventing tampering with any field.

**AEAD construction**: ChaCha20-Poly1305 (RFC 7539, RFC 8439) with the derived 12-byte nonce. The associated data for the AEAD is the envelope header (mode_byte || node_id || sequence). This binds the ciphertext to the specific node, sequence number, and wire mode, so an envelope cannot be replayed with a modified header even if the AEAD passes.

**Overhead computation**:
```
overhead = 1 (mode_byte) + node_id_len + 4 (sequence) + 16 (auth_tag) + 64 (signature)
         = 87 bytes for 2-byte node_id
         = 89 bytes for 4-byte node_id
```
...
### 9.1 Message 1: Capability Advertisement (40 bytes, base or extended form)

| Offset | Size | Field | Description |
|---|---|---|---|
| 0 | 1B | msg_type | 0x01 (ADV base form) or 0x81 (ADV extended form — bit 7 signals `basis_fingerprint` present, `node_id` field narrowed) |
| 1 | 1B | protocol_version | 0x01 |
| 2 | 8B | asd_fingerprint | First 8 bytes of SHA-256(canonical ASD JSON) |
...
| 1 | 1B | match_status | 0x00=exact match (SAIL-capable), 0x01=ASD version mismatch, 0x02=ASD fingerprint mismatch, 0x03=basis mismatch (SAL-only), 0x04=base form meets extended basis (SAL-only) |
| 2 | 8B | echo_asd_fingerprint | ASD fingerprint from received ADV (echo for verification) |
| 10 | 8B | own_asd_fingerprint | Responder's own ASD fingerprint |
...
OSMP defines two related fingerprints, computed by the same hash truncation rule over different inputs.

**ASD Fingerprint.** The ASD fingerprint is the first 8 bytes of SHA-256 over the canonical JSON serialization of the ASD data. Canonical JSON matches Python `json.dumps(data, sort_keys=True)`: keys sorted alphabetically, separators `", "` and `": "`, non-ASCII escaped to `\uXXXX`. All three SDKs (Python, TypeScript, Go) produce identical ASD fingerprints for identical dictionary state.

**Corpus Hash.** The corpus hash for any dictionary corpus (base ASD CSV, MDR CSV, MDR DBLK, or any future corpus format) is the full 32-byte SHA-256 over the corpus file bytes verbatim, with no canonicalization transform.

**Basis Fingerprint.** The basis fingerprint is the first 8 bytes of SHA-256 over the canonical serialization of the ordered basis list. Canonical basis serialization is the concatenation, in basis order, of `corpus_id_length || corpus_id || corpus_hash` for each entry in the basis, where `corpus_id_length` is a single byte giving the length of the UTF-8 `corpus_id` in bytes (1-255), `corpus_id` is the UTF-8 bytes of the identifier, and `corpus_hash` is the 32-byte SHA-256 of the corpus file. All three SDKs produce identical basis fingerprints for identical inputs. Cross-SDK fingerprint equivalence is verified by `tests/test_dictionary_basis.py`.
...
ADV_SENT -> receive ACK (match_status 0x00) -> ESTABLISHED_SAIL
ADV_SENT -> receive ACK (match_status 0x03) -> ESTABLISHED_SAL_ONLY
ADV_SENT -> receive ACK (match_status 0x01 or 0x02) -> SYNC_NEEDED
ADV_SENT -> timeout or invalid response -> FALLBACK
IDLE -> register non-OSMP peer -> FALLBACK
IDLE -> receive ADV -> send ACK -> ESTABLISHED_* or SYNC_NEEDED
...
**Operator policy: `require_sail`.** A node operator MAY configure a `require_sail: true` policy flag. With this flag set, a session that would otherwise enter ESTABLISHED_SAL_ONLY is refused, and the local FNP state machine returns to IDLE. The flag is local-only and intended for homogeneous deployments where any basis mismatch indicates a misconfiguration.

**Session degradation event.** When a session establishes in ESTABLISHED_SAL_ONLY because the peer's basis fingerprint differs from the local node's expected basis fingerprint, the local node logs a session-degradation event. Operator monitoring infrastructure subscribes to these events to detect drift. The event is informational and does not affect session establishment or traffic flow.

### 9.8 Dictionary Basis Manifest

The Dictionary Basis Manifest is the formal specification of the ordered set of dictionary corpora that determines a node's SAIL intern table. It is the runtime representation of the build-time pin established by ADR-001 and extended by ADR-004.
...
**Default basis.** A node that loads only the base ASD has a basis of length one and a well-defined basis fingerprint. This is the default configuration and the configuration assumed by every test vector in `protocol/test-vectors/canonical-test-vectors.json`.

**Forward compatibility.** New corpus types may be added to the basis architecture without changing this section. The basis fingerprint computation depends only on the corpus identifier and the SHA-256 of the corpus file bytes, so new corpus types do not require a basis fingerprint format revision.
...
**Principle 3 — No Meta-Verb Opcodes:** Opcodes encode concepts, parameters, and states — not general-purpose action verbs. Words whose meaning depends entirely on what follows them (ADJUST, MODIFY, UPDATE, CHANGE, SET, CONFIGURE as a generic, and their equivalents) carry no semantic content independent of their object and have no valid opcode instantiation. The operation on a concept is expressed through the instruction structure: slot values supply the target state, the delta glyph supplies the offset, and chained instructions supply the sequence. The opcode names what is being operated on, not the act of operating on it. Example: "adjust temperature by two degrees" composes as `E:TH[Δ+2]` — the opcode names the parameter (TH, threshold/temperature), the glyph names the operation class (Δ, delta/offset), and the slot value names the magnitude.

**Principle 4 — Namespace-Mnemonic Alignment:** A valid dictionary entry exhibits consistent recognition across both scoped presentation (opcode with namespace label) and isolated presentation (opcode without namespace label). When a mnemonic is recognized correctly in isolated mode but recognition collapses under the namespace label, the namespace label and the concept are semantically misaligned. The correct interventions, in priority order, are: (1) reassign the opcode to a namespace whose label does not suppress its meaning, (2) replace the mnemonic with one whose recognition is stable under the current namespace label, or (3) where neither intervention is available, introduce a measurement-validated sibling mnemonic and retain the original for backward compatibility. Retaining an opcode in a namespace whose label actively suppresses its recognition is not a conformant dictionary state.
...
10. **Meta-verb composition:** Composing an instruction whose primary opcode is a general-purpose action verb (ADJUST, MODIFY, UPDATE, CHANGE, SET, or equivalent) with the specific concept supplied only in the slot or target field. The opcode must name the concept being operated on; the slot, Δ glyph, or chained instruction supplies the operation. An instruction of the form ADJUST@[concept] or MODIFY[concept:value] is non-conformant regardless of whether ADJUST or MODIFY appears in the ASD. If such an opcode exists in the local ASD, it is a dictionary defect subject to correction under the Omega replacement procedure; its presence in the ASD does not make its use conformant.
