...
**Two-tier corpus compression** — `D:PACK` applies OSMP encoding followed by LZMA compression for at-rest storage. `D:UNPACK` retrieves semantic content by ASD lookup without requiring full decompression — the zero-unpack semantic resolution property. Empirical result: 72.7% total reduction, 3.7× multiplier versus natural language + LZMA baseline on a partial medical domain corpus.
