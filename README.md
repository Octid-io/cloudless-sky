...
**Two-tier corpus compression** — `D:PACK` applies OSMP encoding followed by LZMA compression for at-rest storage. `D:UNPACK` retrieves semantic content by ASD lookup against the SAL intermediate representation without requiring full corpus expansion. Two complete D:PACK builds demonstrate the architecture at scale: the CMS FY2026 ICD-10-CM code set (74,719 clinical descriptions, 5.6MB raw) produces a 505KB binary (94.9% total reduction), and the ISO 20022 eRepository (66,956 financial message element definitions, 8.7MB raw) produces a 1.14MB binary (90.8% total reduction). Both artifacts fit in ESP32 flash and enable edge-local domain code resolution without network access.
