...
## Finding 49 — Go SDK SAIL encoder rejects `I:§` opcode (pre-existing, surfaced by ADR-004 cross-SDK verification)

**Severity**: Cross-SDK functional bug (single instruction)
**Priority**: Sprint 5 or align with v15 dictionary release
**Status**: FIX APPLIED (post ADR-004 sprint, isolated patch). See `go-wire-f49.go` and `go-fnp-finding49-test.go` in outputs.

**Context**: While verifying cross-SDK SAIL byte equivalence as part of the ADR-004 (Dictionary Basis Manifest) implementation, the new `tests/test_dictionary_basis.py` cross-codec round-trip suite uncovered a divergence between Python/TypeScript and Go on the canonical instruction `K:PAY@RECV↔I:§→K:XFR[AMT]`. Python and TypeScript SDKs round-trip cleanly. Go produces `I:Λ` on decode.

**Root cause**: Two-stage pre-existing bug in `sdk/go/osmp/wire.go`.

**Stage 1 (encode)**: `(*SAILCodec).tryNsOp` at line 721 only accepts opcode characters in the range `[A-Z0-9]`:
...
The TypeScript equivalent in `sdk/typescript/src/osmp_wire.ts` also handles `\xA7`. Go is the outlier. As a result, Go's encoder fails to recognize `I:§` as a namespace:opcode pair, rejects it from `tryNsOp`, and falls through to the alphanumeric-run encode path, which emits 5 bytes (`498bc2a7ff`) instead of the canonical 4-byte SAIL frame `e00806ff` produced by Python and TypeScript.

**Stage 2 (decode collision)**: The 5-byte fallback encoding contains 0xC2 as the first UTF-8 byte of §. SAIL token 0xC2 is `tokATOMIC` (Λ, loss tolerance policy) per the SAIL token table at the top of `wire.go`. When Go's decoder reads its own encoded bytes back, it sees 0xC2 in the token-table range and translates it to Λ. The result: `I:§` round-trips through Go as `I:Λ`. The bytes pass AEAD verification (they were not modified), but the semantic interpretation is wrong.

This is the exact failure pattern Finding 41 was logging at an architectural level — silent SAIL misdecode where the wire bytes verify cleanly but the decoded SAL is wrong. ADR-004 prevents the cross-node version of this failure by anchoring intern table determinism to a basis fingerprint. The Go decoder bug is the within-node version, caused by an opcode character class mismatch, and is orthogonal to the basis manifest architecture.

**Why the existing Go test suite missed it**: `tests/tier1/test_python.py` and `tests/tier1/test_typescript.mjs` run cross-SDK SAIL round-trip tests but the canonical test vector set covered by those tests does not include any instruction containing `I:§`. The Go SAIL test suite at `sdk/go/osmp/` does not exercise the `I:§` opcode at all. The bug has been latent since SAIL shipped.

**The fix**: One-line change to `tryNsOp` in `sdk/go/osmp/wire.go`:
...
The Stage 2 decode collision is a downstream consequence of the encode bug; once the encoder produces the correct 4-byte frame, the decoder's existing TOK_FRAME handling correctly resolves it to `I:§` via the namespace/opcode index tables. No change to the decoder is required.

**Test coverage to add alongside the fix**:

1. Add `I:§` and `K:PAY@RECV↔I:§→K:XFR[AMT]` to the canonical test vector set so all three SDKs exercise the section-sign opcode in the existing tier1 cross-SDK runners.
2. Add a Go-specific unit test in `sdk/go/osmp/wire_test.go` covering every opcode character class permitted by the SAL grammar, not just the alphanumeric subset.
3. Consider an audit pass over `sdk/go/osmp/wire.go` for any other character-class shortcuts that may have drifted from Python's canonical opcode grammar.

**Why this is logged separately from ADR-004**: ADR-004 is the basis manifest architecture for SAIL intern table determinism. This finding is a Go decoder character-class bug. They share a failure mode (silent SAIL misdecode) but the mechanism is unrelated and the fixes don't share code. Mixing them would conflate two unrelated changes in the same release. The ADR-004 sprint ships with the Go decoder bug logged but unfixed; Finding 49 is queued for a dedicated fix sprint, ideally aligned with the v15 dictionary release where Go decoder coverage is going to need attention anyway for the training-convergence sibling opcodes.

**Detection credit**: Surfaced by `tests/test_dictionary_basis.py::TestSAILCodecBasisDeterminism::test_cross_codec_round_trip` and the cross-SDK verification script at `sdk/typescript/tests/cross_sdk_check.ts`. The bug was invisible to the existing test suites because no canonical vector exercised `I:§`. ADR-004's expanded test surface is what made it visible.
