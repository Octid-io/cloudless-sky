...
export const ASD_FLOOR_VERSION = "1.0";

export const GLYPH_OPERATORS: Record<string, { unicode: string; name: string; nl: string[] }> = {
  "∧": { unicode: "U+2227", name: "AND", nl: ["and", "&", "also"] },
  "∨": { unicode: "U+2228", name: "OR", nl: ["or", "either", "alternatively"] },
  "¬": { unicode: "U+00AC", name: "NOT", nl: ["not", "except", "excluding", "without"] },
  "→": { unicode: "U+2192", name: "THEN", nl: ["if...then", "when...then", "provided that", "therefore"] },
  "↔": { unicode: "U+2194", name: "IFF", nl: ["if and only if", "iff", "exactly when"] },
  "∀": { unicode: "U+2200", name: "FOR-ALL", nl: ["for all", "every", "each", "all"] },
  "∃": { unicode: "U+2203", name: "EXISTS", nl: ["any", "there exists", "at least one", "some"] },
  "∥": { unicode: "U+2225", name: "PARALLEL", nl: ["simultaneously", "in parallel", "concurrently"] },
  ">": { unicode: "U+003E", name: "PRIORITY", nl: ["first", "prefer", "prioritize", "greater than"] },
  "~": { unicode: "U+007E", name: "APPROX", nl: ["approximately", "about", "roughly"] },
  "*": { unicode: "U+002A", name: "WILDCARD", nl: ["all", "any value", "broadcast"] },
  ":": { unicode: "U+003A", name: "ASSIGN", nl: ["equals", "is set to", "namespace-separator"] },
  ";": { unicode: "U+003B", name: "SEQUENCE", nl: ["then next", "followed by", "in sequence"] },
  "?": { unicode: "U+003F", name: "QUERY", nl: ["what is", "query", "report"] },
  "@": { unicode: "U+0040", name: "TARGET", nl: ["at", "on", "directed to"] },
  "⟳": { unicode: "U+27F3", name: "REPEAT-EVERY", nl: ["every", "repeat every", "recurring interval"] },
  "≠": { unicode: "U+2260", name: "NOT-EQUAL", nl: ["not equal to", "excluding value", "except value"] },
  "⊕": { unicode: "U+2295", name: "PRIORITY-ORDER", nl: ["in priority order", "ranked", "strict ranked execution"] },
};

export const COMPOUND_OPERATORS: Record<string, { unicode: string; name: string; nl: string[] }> = {
  "¬→": { unicode: "U+00AC U+2192", name: "UNLESS", nl: ["unless", "except when"] },
};

export const CONSEQUENCE_CLASSES: Record<string, { unicode: string; name: string; hitlRequired: boolean }> = {
  "⚠": { unicode: "U+26A0", name: "HAZARDOUS", hitlRequired: true },
  "↺": { unicode: "U+21BA", name: "REVERSIBLE", hitlRequired: false },
  "⊘": { unicode: "U+2298", name: "IRREVERSIBLE", hitlRequired: true },
};

export const OUTCOME_STATES: Record<string, string> = {
  "⊤": "PASS-TRUE",
  "⊥": "FAIL-FALSE",
};

export const PARAMETER_DESIGNATORS: Record<string, { unicode: string; name: string; bytes: number }> = {
