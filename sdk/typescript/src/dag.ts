...
function splitTopLevel(expr: string, sep: string): string[] {
  ...
  while (i < expr.length) {
    const ch = expr[i];
    if (ch === "[" || ch === "(") { depth++; current += ch; i++; }
    else if (ch === "]" || ch === ")") { depth--; current += ch; i++; }
    else if (depth === 0 && expr.slice(i, i + sep.length) === sep) {
      parts.push(current); current = ""; i += sep.length;
    } else { current += ch; i++; }
  }
...
export class DAGFragmenter {
  ...
  private _parseExpr(expr: string, nodes: DAGNode[], parentIndices: number[]): number[] {
    // ; (SEQUENCE) — lowest precedence
    let parts = splitTopLevel(expr, ";");
    if (parts.length > 1) {
      let tails = parentIndices;
      for (const part of parts) tails = this._parseExpr(part.trim(), nodes, tails);
      return tails;
    }
  ...
  fragmentize(compoundSal: string, msgId: number, critical = false): Fragment[] {
    ...
    for (const node of nodes) {
      ...
      if (node.parents.length === 0) {
        ...
        let bitmap = 0;
        for (const p of node.parents) bitmap |= (1 << p);
...
export class DAGReassembler {
  ...
  receive(frag: Fragment): Uint8Array[] | null {
    ...
    if (new TextDecoder().decode(frag.payload).includes("R:ESTOP")) return [frag.payload];
    ...
    if (this.policy === LossPolicy.FAIL_SAFE) {
      ...
      if (isTerminal(frag) && rcv.size === exp) return this._resolveDAG(rcv);
      if (isTerminal(frag)) return this._resolveDAGPartial(rcv);
      return null;
    }
  }

  private _getParents(frag: Fragment): number[] {
    if (frag.flags & FLAG_EXTENDED_DEP) {
      if (frag.payload.length < 4) return [];
      const bitmap = new DataView(frag.payload.buffer, frag.payload.byteOffset, 4).getUint32(0, false);
      const parents: number[] = [];
      for (let i = 0; i < 32; i++) if (bitmap & (1 << i)) parents.push(i);
      return parents;
    }
    // Self-reference = root
    if (frag.dep === frag.fragIdx) return [];
    return [frag.dep];
  }

  private _getPayload(frag: Fragment): Uint8Array {
    if (frag.flags & FLAG_EXTENDED_DEP) return frag.payload.slice(4);
    return frag.payload;
  }

  private _resolveDAG(rcv: Map<number, Fragment>): Uint8Array[] {
    const nodeSet = new Set(rcv.keys());
    const order = this._topoSort(rcv, nodeSet);
    return order.map(i => this._getPayload(rcv.get(i)!));
  }

  private _resolveDAGPartial(rcv: Map<number, Fragment>): Uint8Array[] {
    const present = new Set(rcv.keys());
    const executable = new Set<number>();
    for (const idx of present) {
      if (this._ancestorsSatisfied(rcv, idx, present)) executable.add(idx);
    }
    if (executable.size === 0) return [];
    const order = this._topoSort(rcv, executable);
    return order.map(i => this._getPayload(rcv.get(i)!));
  }

  private _ancestorsSatisfied(rcv: Map<number, Fragment>, idx: number, present: Set<number>): boolean {
    ...
    while (stack.length > 0) {
      ...
      visited.add(current);
      if (!present.has(current)) return false;
      const frag = rcv.get(current);
      if (frag) for (const p of this._getParents(frag)) if (!visited.has(p)) stack.push(p);
    }
    return true;
  }

  private _topoSort(rcv: Map<number, Fragment>, nodeSet: Set<number>): number[] {
    const inDeg = new Map<number, number>();
    const children = new Map<number, number[]>();
    for (const i of nodeSet) { inDeg.set(i, 0); children.set(i, []); }
