...
export class OSMPEncoder {
  ...
  encodeFrame(namespace: string, opcode: string, target?: string, querySlot?: string,
              slots?: Record<string, string | number>, consequenceClass?: string): string {
    ...
    const parts: string[] = [`${namespace}:${opcode}`];
    if (target !== undefined) parts.push(`@${target}`);
    if (querySlot !== undefined) parts.push(`?${querySlot}`);
    if (slots) for (const [k, v] of Object.entries(slots)) parts.push(`:${k}:${v}`);
    if (consequenceClass) parts.push(consequenceClass);
    return parts.join("");
  }
  ...
  encodeSequence(instructions: string[]): string { return instructions.join(";"); }
  encodeBroadcast(namespace: string, opcode: string): string { return `${namespace}:${opcode}@*`; }
}
