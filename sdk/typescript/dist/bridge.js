...
// ── SAL frame detection ────────────────────────────────────────────
import { SAL_FRAME_RE_BRIDGE as SAL_FRAME_RE, NS_PATTERN, OPCODE_PATTERN, } from "./sal_patterns.js";
...
function acquisitionScore(m, threshold) {
...
export class SALBridge {
    ...
    metrics = new Map();
    log = [];
    constructor(nodeId, asd, annotate = true, acquisitionThreshold = DEFAULT_ACQUISITION_THRESHOLD, regressionThreshold = DEFAULT_REGRESSION_THRESHOLD) {
        this.nodeId = nodeId;
    ...
    // ── peer management ────────────────────────────────────────────
    registerPeer(peerId, attemptFnp = true) {
    ...
    getLog(peerId, lastN) {
        let events = this.log;
        if (peerId !== undefined)
            events = events.filter(e => e.remoteId === peerId);
        if (lastN !== undefined)
            events = events.slice(-lastN);
        return events;
    }
    getComparison(peerId) {
        return this.log
    ...
    isPureSal(message) {
        ...
        const frameWithTail = new RegExp("\\b" + NS_PATTERN + ":" + OPCODE_PATTERN
            + "(?:@[A-Za-z0-9_*\\-]+)?"
            + "(?:\\?[A-Za-z0-9_]+)?"
            + "(?:\\[[^\\]]*\\])?"
        ...
        for (const frame of frames) {
            if (!new RegExp(SAL_FRAME_RE.source).test(frame))
                return false;
        }
        return true;
    }
    emit(eventType, remoteId, opts = {}) {
        this.log.push({
            timestamp: Date.now(),
            eventType,
            remoteId,
