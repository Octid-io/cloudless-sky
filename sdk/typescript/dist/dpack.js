...
function findBlock(data, hdr, code) {
    ...
    while (lo <= hi) {
        ...
        while (fcLen > 0 && data[off + fcLen - 1] === 0)
            fcLen--;
...
function decompressBlock(data, hdr, blockIdx) {
...
function searchBlock(decompressed, code) {
 ...
 */
export function resolveBlk(data, code) {
    const hdr = parseHeader(data);
    if (hdr.flags & 1 && hdr.dictSize > 0) {
        throw new Error("This DBLK binary uses a trained dictionary. " +
