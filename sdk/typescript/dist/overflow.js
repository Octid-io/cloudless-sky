...
export function packFragment(f) {
    ...
    buf.writeUInt16BE(f.msgId, 0);
    buf.writeUInt8(f.fragIdx, 2);
    buf.writeUInt8(f.fragCt, 3);
    buf.writeUInt8(f.flags, 4);
    buf.writeUInt8(f.dep, 5);
    buf.set(f.payload, FRAGMENT_HEADER_BYTES);
    return new Uint8Array(buf);
}
export function unpackFragment(data) {
    if (data.length < FRAGMENT_HEADER_BYTES)
        throw new Error(`Fragment too short: ${data.length}`);
...
export function isTerminal(f) { return !!(f.flags & FLAG_TERMINAL); }
export function isCritical(f) { return !!(f.flags & FLAG_CRITICAL); }
export class OverflowProtocol {
    mtu;
    policy;
    _counter = 0;
    _buf = new Map();
    constructor(mtu = LORA_STANDARD_BYTES, policy = LossPolicy.GRACEFUL_DEGRADATION, _timeout = 30) { this.mtu = mtu; this.policy = policy; }
    nextId() { this._counter = (this._counter + 1) % 65536; return this._counter; }
    fragment(payload, critical = false) {
        ...
        const chunks = [];
        for (let i = 0; i < payload.length; i += avail)
            chunks.push(payload.slice(i, i + avail));
        const msgId = this.nextId();
        const fragCt = chunks.length;
    ...
    receive(frag) {
        if (Buffer.from(frag.payload).toString("utf8").includes("R:ESTOP"))
            return frag.payload;
        ...
        if (this.policy === LossPolicy.ATOMIC || isCritical(frag)) {
            ...
            if (isTerminal(frag) && rcv.size === exp)
                return this._reassemble(rcv, exp);
            if (isTerminal(frag))
                return this._reassemblePartial(rcv, exp);
            return null;
        }
        else {
            return rcv.size === exp ? this._reassemble(rcv, exp) : null;
    ...
    _reassemblePartial(rcv, exp) {
        const parts = [];
        for (let i = 0; i < exp; i++) {
            if (!rcv.has(i))
                break;
            parts.push(rcv.get(i).payload);
        }
        const total = parts.reduce((s, p) => s + p.length, 0);
        const out = new Uint8Array(total);
        let off = 0;
