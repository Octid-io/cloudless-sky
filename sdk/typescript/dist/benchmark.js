...
export function runBenchmark(vectorsPath) {
    const resolved = vectorsPath ?? path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../../protocol/test-vectors/canonical-test-vectors.json");
    ...
    for (const vec of data.vectors) {
        ...
        let decodeOk = false;
        try {
            const r = dec.decodeFrame(vec.encoded);
            decodeOk = !!(r.namespace && r.opcode);
        }
        catch {
