...
// ── helpers ─────────────────────────────────────────────────────────
function namespaceBitmap(namespaces) {
    let bitmap = 0;
...
function bitmapToNamespaces(bitmap) {
    const result = [];
    for (let i = 0; i < NS_LETTERS.length; i++) {
        if (bitmap & (1 << i))
            result.push(NS_LETTERS[i]);
    }
    if (bitmap & (1 << 26))
        result.push("\u03A9");
    return result;
}
function fingerprintBytes(asd) {
...
export class FNPSession {
    ...
    requireSail;
    remoteBasisFingerprint = null;
    degradationEvent = null;
    state = "IDLE";
    remoteNodeId = null;
    remoteFingerprint = null;
    commonNamespaces = null;
    matchStatus = null;
    negotiatedCapacity = null;
    constructor(asd, nodeId, asdVersion = 1, channelCapacity = FNP_CAP_FLOOR, opts = {}) {
        this.asd = asd;
        this.nodeId = nodeId;
    ...
    buildAck(remoteFp, match, commonBitmap, negCap) {
        ...
        const isAck = (match === FNP_MATCH_EXACT ||
    ...
    applyMatchToState(match, peerBasisFp) {
        ...
        if (match === FNP_MATCH_BASIS_MISMATCH || match === FNP_MATCH_BASIS_EXT_VS_BASE) {
            ...
            if (this.expectedBasisFingerprint !== null &&
                peerBasisFp !== null &&
