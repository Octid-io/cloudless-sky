...
export class AdaptiveSharedDictionary {
    ...
    _tombstones;
    _versionLog;
    constructor(floorVersion = ASD_FLOOR_VERSION) {
    ...
    lookup(namespace, opcode) {
        if (this._tombstones.has(`${namespace}::${opcode}`))
            return null;
        return this._data[namespace]?.[opcode] ?? null;
    }
    applyDelta(namespace, opcode, definition, mode, versionPointer) {
        ...
        if (mode === DictUpdateMode.ADDITIVE) {
            ...
            if (!(opcode in this._data[namespace]))
    ...
    namespaces() { return Object.keys(this._data).sort(); }
    versionLog() { return [...this._versionLog]; }
}
